import { PositionClassifier } from '../src/common/classifiers/position-classifier';
...
const testCases: TestCase[] = [
  ...
  // 排除词测试
  {
    input: 'Java 前端开发',
    expectedCategory: 'IT 互联网技术',
    expectedConfidenceMin: 0,  // 应该被排除或低分
...
async function runTests() {
  ...
  const classifier = new PositionClassifier();
  
  let passed = 0;
  let failed = 0;
  const failures: { test: string; expected: string; actual: string; confidence: number }[] = [];
